...
import Mask from './HorizontalLineMask';
 ...
 */
export default class EditableHorizontalLine extends EditableShape {
    ...
    if (this.grabbedElem) {
      ...
      if (this.grabbedElem === this.shape) {
        ...
        const updatedPoints = getPoints(this.shape).map((pt, idx) =>
          (idx === handleIdx) ? {'x':pos.x, 'y':pt[1].value} : {'x':pt[0].value, 'y':pt[1].value});

        this.setPoints(updatedPoints);
        this.setHandleX(this.handles[handleIdx], pos.x);
